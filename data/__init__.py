...
from data.sampler import IdentitySampler
from data.sampler import BalanceRandomIdentitySampler
...
def get_train_loader(dataset, root, sample_method, batch_size, p_size, k_size, image_size, random_flip=False, random_crop=False,
    ...
    # sampler
    assert sample_method in ['random', 'identity_uniform', 'balance_random', 'identity_random', 'norm_triplet']
    ...
    elif sample_method == 'balance_random':
        batch_size = p_size * k_size
        sampler = BalanceRandomIdentitySampler(train_dataset, p_size * k_size, k_size)
...
def get_test_loader_new(dataset, root, batch_size, p_size, k_size, image_size, num_workers=4):
    # transform
    transform = T.Compose([
        T.Resize(image_size),
        T.ToTensor(),
    ...
    # dataset
    if dataset == 'sysu':
        test_dataset = SYSUDataset(root, mode='test', transform=transform)
    elif dataset == 'regdb':
        test_dataset = RegDBDataset(root, mode='test', transform=transform)
    elif dataset == 'market':
        gallery_dataset = MarketDataset(root, mode='gallery', transform=transform)
        query_dataset = MarketDataset(root, mode='query', transform=transform)
    batch_size = p_size * k_size
    sampler = IdentitySampler(test_dataset, p_size * k_size, k_size)
    # dataloader
    test_loader = DataLoader(test_dataset, batch_size, sampler=sampler, shuffle=False, drop_last=False, pin_memory=True,
                            collate_fn=collate_fn, num_workers=num_workers)
    return test_loader