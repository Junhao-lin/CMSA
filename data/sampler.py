...
class BalanceRandomIdentitySampler(Sampler): 
    def __init__(self, data_source, batch_size, num_instances, positive_ratio = 0.5):
        self.data_source = data_source
        self.batch_size = batch_size
        self.num_instances = num_instances
        self.num_pids_per_batch = self.batch_size // self.num_instances
    ...
    def __iter__(self):
        ...
        for pid in self.pids:
            ...
            # Positive pairs
            for idx_I, idx_R in zip(idxs_I[:self.num_instances // 2], idxs_R[:self.num_instances // 2]):
                batch_idxs.append(idx_I)
                batch_idxs.append(idx_R)

            # Negative pairs
            other_pids = [other_pid for other_pid in self.pids if other_pid != pid]
            idxs_I_neg = []
            for other_pid in other_pids:
                idxs_I_neg.extend(self.index_dic_I[other_pid])
            np.random.shuffle(idxs_I_neg)

            for idx_I, idx_R in zip(idxs_I_neg[:self.num_instances // 2], idxs_R[self.num_instances // 2:]):
                batch_idxs.append(idx_I)
                batch_idxs.append(idx_R)

            # Shuffle the batch indices to mix positive and negative pairs
            batch_idxs = np.reshape(batch_idxs, (-1, 2)).tolist()
            np.random.shuffle(batch_idxs)
            batch_idxs = [idx for pair in batch_idxs for idx in pair]
