...
from layers import CosineSimilarityLoss
class Baseline(nn.Module):
    def __init__(self, num_classes=None, drop_last_stride=False, pattern_attention=False, modality_attention=0, mutual_learning=False, **kwargs):
        ...
        self.pair_loss = kwargs.get('pair', False)        
    ...
    def forward(self, inputs, labels=None, pair_labels=None, **kwargs):
        loss_reg = 0
        loss_center = 0
        ...
        if self.pattern_attention:
            ...
            if self.training:
                masks = masks.view(b, self.part_num, h * w)
        ...
        if not self.training:
            ...
            return self.train_forward(feats, pair_feat, labels, pair_labels, loss_reg, sub, **kwargs)

    def train_forward(self, feat, pair_feat, labels, pair_labels, loss_reg, sub, **kwargs):
        ...
        if self.pair_loss:
            ...
            pair_loss = self.pair_loss(pair_feat.float(), pair_labels)
            loss += pair_loss
            metric.update({'pair': pair_loss.data})
