...
class CSCA(nn.Module):
    def __init__(self,channels):
        ...
        self.out_channels = self.channels//2
    ...
    def fuse(self, x):
        ...
        fus = torch.cat((RGB_context[0],T_context[0]),0)
        for i in range(1,len(RGB_context)):
            fus1 = torch.cat((RGB_context[i],T_context[i]),0)  
            fus = torch.cat((fus,fus1),0)
        x = fus.view(x.shape)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, zero_init_residual=False, modality_attention=0,
        ...
        #---------------------------------------------------------------Louis add
        self.w1 = nn.Parameter(torch.tensor(2.0),requires_grad=True)
        self.w2 = nn.Parameter(torch.tensor(2.0),requires_grad=True)
        self.w3 = nn.Parameter(torch.tensor(2.0),requires_grad=True)
        self.w4 = nn.Parameter(torch.tensor(2.0),requires_grad=True)
    
        self.part1_CSCA1 = CSCA(256)
        self.part2_CSCA1 = CSCA(256)
        self.part3_CSCA1 = CSCA(256)
        self.part4_CSCA1 = CSCA(256)
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        x = self.layer1(x)
        part_high = int(x.size(2)/4)
        part_x = x.split(part_high ,dim = 2)
        part_x1 = self.part1_CSCA1(part_x[0]) * self.w1
        part_x2 = self.part2_CSCA1(part_x[1]) * self.w2
        part_x3 = self.part3_CSCA1(part_x[2]) * self.w3
        part_x4 = self.part4_CSCA1(part_x[3]) * self.w4
        x = torch.cat([part_x1, part_x2, part_x3, part_x4] ,dim = 2)
